...
function StudyMain() {
  ...
  const filteringData = dataFilter => {
    const filtering = getData().filter(type => type.category === dataFilter);
    return filtering;
  };
  ...
  const handleCategory = e => {
    ...
    if (categoryData !== 'all') setFilterData(filteringData(categoryData));
    else setFilterData(getData());
  };
                ...
                {filterData &&
                  filterData
                    .filter(val => {
                      ...
                      if (val.title.replace(/ /g, '').toLowerCase().includes(search.toLowerCase().replace(/ /g, ''))) {
                        return val;
                      }
                      if (
                        val.subTitle.replace(/ /g, '').toLowerCase().includes(search.toLowerCase().replace(/ /g, ''))
                      ) {
                        return val;
                      }
                      if (
                        val.category.replace(/ /g, '').toLowerCase().includes(search.toLowerCase().replace(/ /g, ''))
                      ) {
                        return val;
                      }
                      return val;
                      ...
                      <Card key={e.title} className={styles.card}>
                        ...
                        <Card.Body className={styles.cardBody}>
                          ...
                          <div className={styles.postImage}>
                            <img src={`http://127.0.0.1:8000${e.image}`} alt="사진없음" />
                          </div>
                        </Card.Body>
                      </Card>
                    ))}
